...
import { DribbbleIcon, GithubIcon, LinkedInIcon, MoonIcon, SunIcon, TwitterIcon,Whatsapp } from './Icons'
...
const CustomLink = ({href,title,className=""}) =>{
    ...
    return(
        ...
        <span className={`h-[1px] inline-block
         ...
         ease duration-300 dark:bg-light ${router.asPath === href ? 'w-full' : 'w-0'}`}>&nbsp;</span>
        </Link>
    )
}
...
const CustomMobileLink = ({href,title,className="",toggle}) =>{
  ...
  return(
      ...
      relative group text-light dark:text-dark my-2`} onClick={handleClick}>
      {title}

      <span className={`h-[1px] inline-block
       bg-light dark:bg-dark absolute left-0 -bottom-0.5 
       group-hover:w-full transition-[width] 
       ease duration-300  ${router.asPath === href ? 'w-full' : 'w-0'}`}>&nbsp;</span>
      </button>
  )
}
...
<motion.div initial={{scale:0,opacity:0,x:"-50%", y:"-50%"}} animate={{scale:1,opacity:1}} className='min-w-[70vw] z-30
