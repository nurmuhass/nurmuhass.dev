...
const Skill = ({name,x,y}) =>{
    return(
        ...
        rounded-full font-semibold bg-dark text-light
         p-8 py-3 px-6 shadow-dark cursor-pointer absolute dark:text-dark dark:bg-light
         lg:py-2 lg:px-4
         md:text-sm md:py-1.5 md:px-3 xs:bg-transparent xs:dark:bg-transparent
          xs:text-dark xs:dark:text-light xs:font-bold' 
         whileHover={{scale:1.05}} initial={{x:0,y:0}} 
...
const Skills = () => {
  return (
    <>
    <h2 className='font-bold text-8xl mt-32 w-full text-center 
    md:text-6xl md:mt-22'>Skills</h2>
    ...
    <motion.div className='flex items-center justify-center 
        rounded-full font-semibold bg-dark text-light p-8 shadow-dark
         cursor-pointer  dark:text-dark dark:bg-light lg:p-6 md:p-4 xs:text-xs xs:p-2' whileHover={{scale:1.05}}>
        Web
        </motion.div>
        <Skill name="JAVASCRIPT"  x="-25vw" y="2vw"/>
        <Skill name="HTML & CSS"  x="-5vw" y="-10vw"/>
        <Skill name="PHP"  x="20vw" y="6vw"/>
        <Skill name="REACT NATIVE"  x="0vw" y="12vw"/>
        <Skill name="REACT JS"  x="-20vw" y="-15vw"/>
        <Skill name="NODE JS"  x="-25vw" y="-25vw"/>
